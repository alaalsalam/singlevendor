...
frappe.ui.form.on('Discounts', {
    ...
    filter_link_fields_data(frm){
        frm.set_query("items", "discount_products", function(doc, cdt, cdn) {
            return {
                'query': 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.get_products'
            };
        });
        frm.set_query("free_product", function(doc) {
            let lists = frm.doc.discount_products.map(val => val.items)
            return {
                'query': 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.get_free_item',
                'filters': {
                    'items': lists
                }
            }
    ...
    hide_recurring_order_sec(frm){
        frappe.call({
            method: `ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.get_shopping_cart_settings`,
            args: {
            },
            callback: function(r) {
                if (r.message) {
        ...
        if (frappe.session.user == 'Administrator' && !frm.doc.__islocal) {
            frm.add_custom_button(__('Save as template'), function() {
                frappe.prompt([
                    ...
                    function(value) {
                        frappe.call({
                            method: `ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.save_as_template`,
                            args: {
    ...
    get_templates: function(frm) {
        frappe.call({
            method: `ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discount_template.discount_template.get_all_templates`,
            args: {},
            async: false,
            callback: function(r) {
    ...
    check_for_cashback: function(frm) {
        frappe.xcall('ecommerce_business_store_singlevendor.utils.setup.get_settings_from_domain', 
...
frappe.ui.form.on("Discount Products", {
    form_render: function(frm, cdt, cdn){
        let items = locals[cdt][cdn];
       if(items.items){
           frappe.xcall(`ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.get_product_attribute`, { "product": items.items})
            .then(r => {
                let option_list = [];
                ...
                $.each(r, function(i, p){
                 ...
                 var possible_val = [{
                        ...
                        "search_fields": "item",
                        "reference_method": `ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.get_product_attribute`,
                        "is_child": 0,
...
frappe.ui.form.on("Discount Applied Product", {
    form_render: function(frm, cdt, cdn){
        let items = locals[cdt][cdn];
       if(items.product){
           frappe.xcall(`ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.get_product_attribute`, { "product": items.product})
            .then(r => {
                let option_list = []
                $.each(r, function(i, p){
                    $.each(p.options, function(j, q){
    ...
    save_action: function() {
        ...
        this.dialog.fields_dict.add_item.onclick = function() {
            ...
            let input = frappe.ui.form.make_control({
                ...
                "get_query": function() {
                    if (me.dt == 'Product')
                        return "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts.get_products"
                },
...
var DiscountWizard = Class.extend({
    ...
    get_template_detail: function() {
        let me = this;
        frappe.call({
            method: `ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discount_template.discount_template.get_template_info`,
            args: {
                name: me.template
            },
            async: false,
...
var SetupWizard = class SetupWizard extends frappe.ui.Slides {
    ...
    add_new_discount() {
        ...
        frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.insert_doc',
            args: {
                doc: this.values
            },
            callback: function(r) {
