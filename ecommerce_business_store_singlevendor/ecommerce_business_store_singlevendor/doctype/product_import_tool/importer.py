...
class Importer:
	...
	def process_doc(self, doc):
		if self.import_type == INSERT:
			p_attributes = doc.get("product_variant_option_mapping")
			if doc.get("product_variant_option_mapping") is not None:
				del doc['product_variant_option_mapping'] 
			return self.insert_record(doc,p_attributes)
		elif self.import_type == UPDATE:
			return self.update_record(doc)

	def insert_record(self,doc,p_attributes):
		...
		new_doc.insert()
		if p_attributes:
			self.insert_product_attribute(new_doc,p_attributes)
		return new_doc

	def insert_product_attribute(self,product,p_attributes):
		idx = 0
		product_attributes = []
		for x in p_attributes:
			p_attr = frappe.db.get_all("Product Attribute",filters={"name":x.attribute},fields=['attribute_name','unique_name'])
			if p_attr:

				p_attr_doc = frappe.new_doc("Product Attribute Mapping")
				p_attr_doc.parent = product.name
				p_attr_doc.parentfield = "product_attributes"
				p_attr_doc.parenttype = "Product"
				p_attr_doc.attribute = p_attr[0].attribute_name
				p_attr_doc.attribute_unique_name = p_attr[0].unique_name
				p_attr_doc.control_type = "Radio Button List"
				p_attr_doc.display_order = idx
				p_attr_doc.is_required = "Yes"
				p_attr_doc.product_attribute = x.attribute
				p_attr_doc.save(ignore_permissions=True)
				product_attribute = {
							"product": product.name,
							"attribute": p_attr[0].attribute_name,
							"control_type": "Radio Button List",
							"display_order": idx,
							"is_required": "Yes",
							"product_attribute": x.attribute,
							"name":p_attr_doc.name
				}	
				...
				if options:
					...
					for opt in options:
						if opt:
							p_attr_opt_doc = frappe.new_doc("Product Attribute Option")
							p_attr_opt_doc.parent = product.name
							p_attr_opt_doc.parentfield = "attribute_options"
							p_attr_opt_doc.parenttype = "Product"
							p_attr_opt_doc.attribute = x.attribute
							p_attr_opt_doc.attribute_id = p_attr_doc.name
							p_attr_opt_doc.option_value = opt
							p_attr_opt_doc.is_pre_selected = 1 if opt_idx==0 else 0
							p_attr_opt_doc.disable = 0
							p_attr_opt_doc.display_order = opt_idx
							p_attr_opt_doc.save(ignore_permissions=True)
							opt_idx = opt_idx+1
							product_attribute["attroptions"].append(p_attr_opt_doc)
				idx = idx+1
				product_attributes.append(product_attribute)
		
		combinations = self.generate_combinations(product.name,product_attributes)
		for c in combinations:
			p_comb_doc = frappe.new_doc("Product Variant Combination")
			p_comb_doc.parent = product.name
			p_comb_doc.parentfield = "variant_combination"
			p_comb_doc.parenttype = "Product"
			p_comb_doc.price_adjustment = 0
			p_comb_doc.product_title = c.get("product_title")
			p_comb_doc.attributes_json = json.dumps(c.get("attributes_json"))
			p_comb_doc.attribute_id = c.get("attribute_id")
			p_comb_doc.attribute_html = c.get("attribute_html")
			p_comb_doc.sku = c.get("sku")
			p_comb_doc.show_in_market_place = 1
			p_comb_doc.save(ignore_permissions=True)
		
	def generate_combinations(self,product_id,attributes):
		options=[]
		all_options=[]
		import json
		response=attributes
		p_combinations = frappe.db.get_all("Product Variant Combination",filters={"parent":product_id},fields=['attribute_id'])
		...
		lists=[]
		sku_no = 1
		sku_seq = frappe.db.get_all("Product Variant Combination",filters={"parent":product_id},fields=['sku'],order_by="sku desc",limit_page_length=1)
		if sku_seq:
			if sku_seq[0].sku:
				l_sku = sku_seq[0].sku
		...
		for item in combination:
			...
			variant_text = ""
			attributes_json = []
			# inserted by siva on 27-sep-2021
			for li in item:
				...
				if check and check.get('name'):
					...
					# html+='<tr><td><div class="col-md-12" style="padding: 5px;border: 1px solid #d1d8dd;">'+attribute_name+'<br>'+li+'</div></td></tr>'
			...
			if ids:
				# attributes_json.sort()
				if not any(obj['attribute_id'] == ids for obj in p_combinations):
					v_sku = product_id.split("ITEM-")[1]+'%03d' % sku_no
					# frappe.log_error(title="v_sku",message=v_sku)
					p_title = frappe.db.get_value("Product",product_id,"item")+" - "+variant_text[:-1]
					lists.append({'sku':v_sku,'product_title':p_title,'attribute_html':html,'attribute_id':ids, 'stock': 1,'price':price, 'attributes_json': attributes_json})
					sku_no = sku_no+1
			else:
	...
	def update_record(self, doc):
		...
		if get_diff(existing_doc, updated_doc):
			...
			updated_doc.save()
			if p_attributes:
				frappe.db.sql("DELETE FROM `tabProduct Variant Combination` WHERE parent=%(product_id)s",{"product_id":doc.get("name")})
				frappe.db.sql("DELETE FROM `tabProduct Attribute Option` WHERE parent=%(product_id)s",{"product_id":doc.get("name")})
				frappe.db.sql("DELETE FROM `tabProduct Attribute Mapping` WHERE parent=%(product_id)s",{"product_id":doc.get("name")})
				frappe.db.commit()
				self.insert_product_attribute(updated_doc,p_attributes)
			return updated_doc
		else:
			# throw if no changes
...
def create_import_log(data_import, log_index, log_details):
	frappe.get_doc(
		{
			"doctype": "Product Import Tool Log",
			"log_index": log_index,
