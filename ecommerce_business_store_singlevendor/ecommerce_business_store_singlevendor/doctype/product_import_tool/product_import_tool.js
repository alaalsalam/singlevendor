...
frappe.ui.form.on("Product Import Tool", {
	setup(frm){
		frappe.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			frappe.model.clear_doc("Product Import Tool", frm.doc.name);
			frappe.model.with_doc("Product Import Tool", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
	...
	download_template(frm) {
            var url = "/assets/ecommerce_business_store_singlevendor/product_import_template.xlsx";
            ...
            req.onload = function () {
                ...
                if (isIE) {
                    window.navigator.msSaveBlob(blob, "product_import_template.xlsx");
                } else {
                    var url = window.URL || window.webkitURL;
                    link = url.createObjectURL(blob);
                    var a = document.createElement("a");
                    a.setAttribute("download", "product_import_template.xlsx");
	...
	show_import_warnings(frm, preview_data) {
		$('[id="page-Customers Import Tool"] [data-fieldname="import_warnings_section"]').attr("style","background: #f8f8f8;")
		...
		html += Object.keys(warnings_by_row)
			.map((row_number) => {
				...
				<div class="warning" data-row="${row_number}">
					<h5 class="text-uppercase" style=""color:#09abdb">${__("Row {0}", [row_number])}</h5>
					<div class="body"  style=""color:#09abdb"><ul>${message}</ul></div>
				</div>
			`;
			})
			.join("");
			...
			.map((warning) => {
				...
				return `
					<div class="warning" data-col="${warning.col}">
						...
						<div class="body" style="color:red">${warning.message}</div>
					</div>
				`;
			})
			.join("");
						...
						if (log.success) {
							...
							html = `${messages}
								...
								<div class="collapse" id="${id}" style="margin-top: 15px;">
									<div class="well">
										<pre style="color: red">${log.exception}</pre>
									</div>
								</div>`;
						}
						let indicator_color = log.success ? "green" : "red";
						let title = log.success ? __("Success") : __("Failure");
						let rowstyle=log.success?'style="color: green;"':'style="color: red;"';
