...
from ecommerce_business_store_singlevendor.utils.setup import get_settings_from_domain, get_settings_value_from_domain, get_doc_versions
from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import update_status, get_today_date
from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.customers.customers import update_customer_preference_from_order
from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import node_cancel_order, node_unassigned_driver_order
...
class Order(Document):
	...
	def on_update(self):		
		if self.order_item:
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts import check_product_discounts
			total_weight = 0
		...
		if check_domain("restaurant") and self.docstatus == 0:
			if self.business and self.order_time != "ASAP":
				...
				from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import check_business_closed
	...
	def calculate_discount(self, order_settings):
		...
		if self.order_item and len(self.order_item) > 0 and order_settings.allow_admin_to_edit_order == 1 and self.is_custom_discount == 0 and allowed_discount == 1:
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts import get_order_subtotal_discount
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import validate_coupon
			total_weight = 0
			for item in self.order_item:
				total_weight += flt(item.weight) if item.weight else 0
			...
			if discount_response and discount_response.get('discount_amount'):
				from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import insert_discount_usage_history
	...
	def update_items_stock(self):
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import get_attributes_json
		try:
			if self.name:
	...
	def on_submit(self):
		...
		frappe.enqueue('ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.clear_guest_user_details', order_id=self.name)
		if self.status == 'Cancelled':
	...
	def update_coupon_discount(self):
		...
		for item in self.order_item:
			total_weight += flt(item.weight) if item.weight else 0
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts import get_coupon_code
		...
		frappe.log_error(response, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.update_coupon_discount')
	...
	def check_order_delivery(self):
		...
		if not orderDelivery:
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import notify_drivers
				...
				if item.next_state == 'Driver Accepted':
					...
					if allow_notification:
						...
						if cancel_order:
							...
							node_cancel_order(self.name, cancel_at, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.cancel_order', cancel_after, business=self.business)
							...
							if not enable_driver_reallocation:
								...
								current_time = datetime.datetime.now()
								cancel_at = current_time + timedelta(seconds=int(cancel_after))
								node_unassigned_driver_order(self.name, cancel_at, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.driver_unassigned_order', cancel_after, business=self.business)
				...
				if check_reallocation:
					...
					if notifications:
						...
						for x in range(reallocate_drivers_max_count):
							node_cancel_order(self.name, schedule_at, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.retrigger_notify_drivers', time_inseconds, business=self.business)
					...
					if diff <= (int(notify_time) * 60):
						...
						if int(diff) > 0:
							...
							if self.restaurant_status == 'Accepted' and self.workflow_state in ['Restaurant Accepted', 'Business Accepted']:
								node_cancel_order(self.name, new_date, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.send_business_notification', diff, business=self.business)
							if self.shipping_method_name == 'Delivery' and self.driver_status != 'Accepted' and self.workflow_state != 'Driver Not Assigned':
								node_cancel_order(self.name, new_date, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.reschedule_notification', diff, business=self.business)
		...
		if self.restaurant_status == 'Pending':
			...
			if allow_cancel_order:
				...
				if cancel_order_business:
					...
					cancel_at = current_time + timedelta(seconds=int(cancel_after))						
					node_cancel_order(self.name, cancel_at, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.cancel_order', cancel_after, business=self.business)
	...
	def calculate_travel_time(self):
		try:
			if self.shipping_method_name == 'Delivery':
				...
				from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import check_distance_from_maps
			...
			frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order(calculate_travel_time)')
	...
	def check_cashback(self):
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.discounts.discounts import update_customer_wallet
...
def calculate_order_shipping_charges(self):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.calculate_order_shipping_charges')
...
def update_wallet_entry(self):		
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.update_wallet_entry") 
...
def get_drivers(doctype, txt, searchfield, start, page_len, filters):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.get_drivers") 
...
def get_customers(doctype,txt,searchfield,start,page_len,filters):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.get_customers") 
...
def get_order_status(doctype,txt,searchfield,start,page_len,filters):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.get_order_status") 
...
def check_payment(self):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.check_payment") 
...
def send_cancel_notification(self):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.send_cancel_notification") 
...
def send_notification(self,doctype="Order",message="You have an order!"):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.send_notification") 
...
def check_orderDelivery(self):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.check_orderDelivery") 
...
def update_orderDelivery(self):
	try:
		...
		frappe.log_error(frappe.get_traceback(), "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.update_orderDelivery") 



@frappe.whitelist()
def make_invoice(source_name, target_doc=None):
	from ecommerce_business_store_singlevendor.accounts.api import make_sales_invoice
...
def cancel_order(order):
	...
	if OrderDetail.restaurant_status != 'Accepted' and OrderDetail.status != 'Cancelled':
		...
		if "mobile_notification" in frappe.get_installed_apps():
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import send_notification_for_order
				...
				if check_state:
					...
					if check_driver_status:
						...
						if cancel_order and int(cancel_order) == 1:
							...
							current_time = datetime.now()
							cancel_at = current_time + timedelta(seconds=int(cancel_after))
							node_cancel_order(order_doc.name, cancel_at, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.cancel_order', cancel_after, business=order_doc.business)							
						else:
							pass
							cancel_after = get_settings_value_from_domain('Business Setting','driver_unassigned_after', domain=domain)
							if order_doc.workflow_state != 'Driver Not Assigned' and cancel_after and int(cancel_after) > 0:
								current_time = datetime.now()
								cancel_at = current_time + timedelta(seconds=int(cancel_after))
								node_unassigned_driver_order(order_doc.name, cancel_at, 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.driver_unassigned_order', cancel_after, business=order_doc.business)
		return {'status': 'Success'}
	except Exception as e:
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.reschedule_notification')
		return {'status': 'Error'}
...
def send_business_notification(order):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.send_business_notification')
...
def calculate_multi_vendor_fixed_rate_shipping_charges1(
	...
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.calculate_multi_vendor_fixed_rate_shipping_charges')
