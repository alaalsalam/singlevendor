...
frappe.require("assets/ecommerce_business_store_singlevendor/js/frappe-datatable.min.js");
frappe.require("assets/ecommerce_business_store_singlevendor/js/frappe-datatable.min.css");
...
frappe.ui.form.on('Order', {
    ...
    customer: function(frm) {
        if (frm.doc.customer) {
            frappe.call({
                method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.get_doc_single_value',
                args: {
        ...
        if (frm.doc.customer){
            frappe.call({
                method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.get_customer_details',
                args: {
                    'doctype': 'Customers',
        ...
        if(frm.doc.status == "Packed" || frm.doc.status == "Ready to Ship" || frm.doc.status == "In Process"){
            ...
            dialog.set_primary_action(__('Save'), function() {
                let values = dialog.get_values();
                    ...
                    frappe.call({
                        method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.update_order_items_status',
                        args: {
    ...
    get_all_order_items_list: function(frm, type) {
        if(frm.doc.order_item.length<=0){
            ...
            frappe.call({
                method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order._get_order_items_',
    ...
    get_all_order_items: function(frm, type) {
       if(frm.doc.order_item.length<=0){
           frappe.throw("Please choose item.")
            ...
            frappe.call({
                    method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.get_order_items',
                    args: { "OrderId": frm.doc.name, 'fntype': type },
                    async: false,
                    callback: function(data) {
                        refresh_field("order_item");
    ...
    create_return: function(frm) {
        ...
        frm.return_dialog.set_primary_action(__('Create Return'), function() {
            ...
            if (lists.length > 0 && allow_form == true) {
                frappe.call({
                    method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.orders.create_return_request',
                    args: {
                        data : val_data
                    },
                    callback: function(d) {
    ...
    get_available_checkout_attributes: function(frm) {
        frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.orders.get_checkout_attributes',
            args: {
...
frappe.ui.form.on("Order Item", {
    ...
    varaint_txt: function(frm, cdt, cdn) {
        var d = frappe.get_doc(cdt, cdn);
        frappe.call({
            method: "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.update_variant_id",
            args: {
                product: d.item,
    ...
    item: function(frm, cdt, cdn) {
        ...
        if(d.item && frm.doc.customer){
              frappe.call({
                method: "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.get_product_price",
...
var change_status = function(docname, order_status, restaurant_status, driver_status) {
    frappe.call({
        method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.orders.update_order_status',
        args: {
            name: docname,
                    ...
                    if (allow) {
                        ...
                        if(frm.check_delivery == 0 && (frm.doc.status == 'Packed'|| frm.doc.status == 'In Process')){
                            frm.add_custom_button(__("Mark As Ready"), function() {
                                frappe.call({
                                method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.mark_as_ready',
