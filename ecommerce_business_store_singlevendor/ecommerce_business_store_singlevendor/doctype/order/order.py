...
class Order(Document):
	...
	def on_update(self):
		...
		if self.order_item:
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.\
												discounts.discounts import check_product_discounts
			total_weight = 0
	...
	def calculate_items_discount(self):
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.\
			discounts.discounts import get_product_discount
		new_items_discount = 0
		for item in self.order_item:
			d_amount = 0
		...
		for x in self.order_item:
			if x.discount and x.is_free_item == 0:
				...
				if not discount_usage_history:
					from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.\
						orders import insert_discount_usage_history
					insert_discount_usage_history(
													self.name, 
													self.customer, 
													x.discount
	...
	def check_sub_total_discounut(self, total_weight):
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.\
			doctype.discounts.discounts import get_order_subtotal_discount
	...
	def on_submit(self):
		...
		frappe.enqueue('ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order.order.clear_guest_user_details',
						order_id = self.name)
	def on_cancel(self):
			...
			for item in self.order_item:
				if item.order_item_type == 'Product' and frappe.db.exists("Product", item.item):
					...
					if product_info.inventory_method=='Track Inventory':
						...
						from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.\
							v2.orders import get_attributes_json
	...
	def update_coupon_discount(self,cart_items):
		...
		for item in self.order_item:
			total_weight += flt(item.weight) if item.weight else 0
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.\
			doctype.discounts.discounts import get_coupon_code
	...
	def check_cashback(self):
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.\
			discounts.discounts import update_customer_wallet
...
def update_discount_foradminorder(doc):
	...
	from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.\
		discounts.discounts import get_order_subtotal_discount
...
def order_detail_tmplt_delivery_slot(items,order_info):
	order_info[0].order_item = items
	checkout_attributes = frappe.db.get_all('Order Checkout Attributes',
	...
	themes = frappe.db.get_all('Web Theme', 
									filters = {'is_active': 1},
									fields = ['primary_button_background_color'])
	if themes:
		theme_settings = themes[0].primary_button_background_color
	delivery_slot = []
	check_delivery_slot = frappe.db.get_all('Order Delivery Slot',
									filters = {'order': order_info[0].name},
									fields = ['order_date', 'from_time', 'to_time','product_category'])
	if check_delivery_slot:
		for deli_slot in check_delivery_slot:
			category = ''
	...
	return [order_details,show_address,theme_settings,delivery_slot]
...
def generate_order_detail_template(order_id):
	catalog_settings = frappe.get_single('Catalog Settings')
	currency = frappe.db.get_all('Currency',
										fields = ["*"], 
										filters = {"name":catalog_settings.default_currency},
										limit_page_length = 1)
	if order_id:
		order_info = frappe.db.get_all('Order', 
										fields = ['*'], 
										filters = {'name':order_id})
		if order_info:
			items = frappe.db.get_all('Order Item',
										filters = {'parent':order_info[0].name},
										fields = ['*'],
										order_by = 'idx')
			if items:
				order_detail_template_items(items)

			data = order_detail_tmplt_delivery_slot(items,order_info)
			order_details = data[0]
			show_address = data[1]
			theme_settings = data[2]
			delivery_slot = data[3]
			_data = order_shipment_status(order_info,order_details)
			order_settings = _data[0],
			shipments = _data[1],
			status_history = _data[2],
			tax_splitup = _data[3],
			check_process_status = _data[4],
			allow_shipment = _data[5],
			customer_fullname = _data[6],
			customer_notes = _data[7],
			farm_share_notes = _data[8],
			is_driver = _data[9]
			additional_charges = frappe.db.get_all("Order Additional Charge",
													filters = {"parent":order_details.name},
													fields = ['charge_name','amount'])
			customer_info = frappe.get_doc("Customers",order_details.customer)
			if not additional_charges:
				add_charges = frappe.db.get_all("Market Place Fee",
										fields = ["fee_name","fee_amount","fee_type","fee_percentage"])
				if add_charges:
					for a_charge in add_charges:
						a_charge_amt = a_charge.fee_amount
						if a_charge.fee_type=="Percentage":
							a_charge_amt = order_details.order_subtotal*a_charge.fee_percentage/100
						additional_charges.append({
													"charge_name":a_charge.fee_name,
													"amount":a_charge_amt
												})
						frappe.get_doc({
											"doctype":"Order Additional Charge",
											"parent":order_details.name,
											"parenttype":"Order",
											"parentfield":"order_additional_charges",
											"charge_name":a_charge.fee_name,
											"amount":a_charge_amt,
										}).save(ignore_permissions=True)
			context = {
