...
frappe.provide("ecommerce_business_store_singlevendor.wallet");
frappe.ui.form.on('Wallet', {
    refresh: function(frm) {
        ...
        if(!frm.doc.__islocal){
            ...
            new ecommerce_business_store_singlevendor.WalletOrders({
                page_len: 10,
                counter_page_len:10,
                user: frm.doc.user,
                type:"Transaction"
    ...
    get_counter_apy_counters(frm){
        if(frm.doc.user){
            frappe.call({
                method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.wallet.wallet.get_counter_apy_counters',
    ...
    get_wallet_settings(frm){
        frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.wallet.wallet.get_wallet_settings',
            args: {},
            async: false,
            callback: function(data) {
                if (data.message) {
...
ecommerce_business_store_singlevendor.WalletOrders = Class.extend({
    init: function(opts) {
        this.opts = opts;
        this.order_list = [];
    ...
    get_transaction_list: function(){
        var me = this;
        frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.wallet.wallet.get_transaction_history',
            args: me.args,
            async: false,
            callback: function(data) {
                if (data.message) {
    ...
    get_order_list: function() {
        var me = this;
        frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.wallet.wallet.get_all_orders',
            args: me.args,
            async: false,
            callback: function(data) {
                if (data.message) {
    ...
    get_commission_list: function() {
        var me = this;
        frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.wallet.wallet.get_commission_list',
            args: me.args,
            async: false,
            callback: function(data) {
                if (data.message) {
     ...
     make_dialog: function() {
       ...
       set_dialog_primary_action: function() {
        ...
        me.dialog.set_primary_action(__('Confirm'), function() {
            frappe.call({
				method: "ecommerce_business_store_singlevendor.accounts.api.make_withdraw_request",
				args: {
    ...
    set_fund_dialog_primary_action: function() {
        ...
        me.dialog.set_primary_action(__('Confirm'), function() {
            frappe.call({
				method: "ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.wallet.wallet.add_fund_to_wallet",
				args: {
					source_name: cur_frm.doc.name,
				},
				callback: function(r) {
...
var apex_chart = function() {
    if (cur_frm.chart_options) {
         frappe.require("assets/dashboard/assets/apexcharts/apexcharts.min.js", function() {
            setTimeout(function() {
                console.log($('#chart_option')[0])
                var charts = new ApexCharts(
                    $('#chart_option')[0],
                    cur_frm.chart_options.chart_options
                );
                charts.render();
            }, 600)
        });   
