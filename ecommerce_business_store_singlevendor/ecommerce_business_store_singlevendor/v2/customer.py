...
from ecommerce_business_store_singlevendor.utils.utils import \
...
def update_password(new_password, logout_all_sessions=0, key=None, old_password=None, user=None):
	try:
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.order_settings.order_settings import validate_password
...
def update_device_id(device_id):
	try:
		customer = get_customer_from_token()
		if customer:
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.common import update_device_id
