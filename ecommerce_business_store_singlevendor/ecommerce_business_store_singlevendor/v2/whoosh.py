	...
	if doc.order_item:
		for x in doc.order_item:
			...
			from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.\
				product.product import update_whoose_search
...
def update_products(doc):
	for x in doc.get('items'):
		...
		frappe.enqueue('ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.product.product.update_whoose_search',self = pr_doc)
