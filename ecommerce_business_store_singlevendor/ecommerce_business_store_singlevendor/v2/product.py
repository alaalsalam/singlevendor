...
def get_parent_categories(domain=None,business=None):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.get_parent_categories')
...
def delete_attribute_image(deleted_doc):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.delete_attribute_image')
...
def product_details(product,customer_id):
	from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.\
		product.product import get_product_reviews_list
...
def attribute_options(attribute,x,op,count,attribute_ids,customer,has_attr_stock):
	...
	if x.inventory_method == 'Track Inventory By Product Attributes':
		attribute_ids=op.name+'\n'
		from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.cart import validate_attributes_stock
...
def get_bestsellers(limit=None, isMobile=0):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.get_bestsellers')
