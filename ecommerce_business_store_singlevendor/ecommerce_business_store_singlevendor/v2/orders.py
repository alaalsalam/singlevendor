...
from ecommerce_business_store_singlevendor.utils.utils import role_auth,check_mandatory_exception,\
...
def insert_order(data):
    ...
    if val_ip and customer_id:
        ...
        if order_settings.enable_zipcode:
            from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.checkout\
...
def get_order_invoices(order_id):
    ...
    if customer_id == frappe.db.get_value("Order",order_id,"customer"):
        ...
        if not token:
            ...
            frappe.db.commit()
        url_string = frappe.utils.get_url()+'/api/method/ecommerce_business_store_singlevendor.\
                            ecommerce_business_store_singlevendor.v2.orders.get_invoices?token='+str(token)
        return {
                "status":"Success",
                "url":url_string
            }
