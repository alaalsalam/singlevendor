...
class SalesInvoice(Document):
	...
	def on_submit(self):
		from ecommerce_business_store_singlevendor.accounts.api import make_invoice_payment
		if frappe.db.exists("Order", self.reference):
			doc = frappe.get_doc("Order", self.reference)
		elif frappe.db.exists("Vendor Orders", self.reference):
			doc = frappe.get_doc("Vendor Orders", self.reference)
