	...
	$.getScript('https://releases.transloadit.com/uppy/v1.22.0/uppy.min.js', function () {
	 loadCss("https://releases.transloadit.com/uppy/v1.22.0/uppy.min.css")
		...
		var uppy = Uppy.Core({
			...
			.use(XHRUpload, {
				endpoint: window.location.origin + '/api/method/ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.product.product.upload_img',
				method: 'post',
				formData: true,
				fieldname: 'files[]',
				headers: {
...
function update_image(r, s, upload_doc, files_count, count, parentfield, image_doctype){
	...
	frappe.call({
		method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.v2.product.update_to_file',
		args: {
...
function update_attribute_image(filename, doctype, docname, count, files_count){
	frappe.call({
		method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.update_attribute_images',
		args:{
			name: filename,
			dt: doctype,
			dn: docname
