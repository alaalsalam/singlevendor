...
function openImageModel(e){
   $('#mains').css("width", '68%');
	...
	console.log(cur_frm.docname)
  	frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.product.product.get_gallery_list',
            args: {
...
function saveImageModel(e){
	...
	if(dataname){
		 frappe.call({
            method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.doctype.product.product.save_gallery_changes',
            args: {
                "data": {
