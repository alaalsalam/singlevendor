...
frappe.ui.form.Form.prototype.savecancel = function(btn, callback, on_error) {
  console.log("---savecancel---")
    const me = this;
    frappe.call({
        method: "ecommerce_business_store_singlevendor.accounts.defaults.get_submitted_linked_docs",
        args: {
...
frappe.ui.form.Form.prototype._cancel_all = function(r, btn, callback, on_error) {
    ...
    if (can_cancel) {
        d.set_primary_action("Cancel All", () => {
            d.hide();
            frappe.call({
                method: "ecommerce_business_store_singlevendor.accounts.defaults.cancel_all_linked_docs",
                args: {
                    docs: links
                },
                freeze: true,
