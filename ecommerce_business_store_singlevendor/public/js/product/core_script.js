...
function add_translation() {
    ...
    var field_option = [];
    frappe.call({
        method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.get_fields_label_with_options',
...
function update_translations(data) {
    return frappe.call({
        method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.update_translations_for_docpage',
...
function option_selection(e){
    ...
    if(val){
       ...
       if(oindex<0){
        ...
        optionhtmls +='" onclick="pick_color($(this))" ondblclick="update_attroption($(this))"><img src="/assets/ecommerce_business_store_singlevendor/images/section-icon.svg" style="height:10px;cursor: all-scroll;position: relative;">'+val+'</a>'
...
$(document).ready(function() {
    frappe.require("/assets/ecommerce_business_store_singlevendor/css/ui/jodit.min.css");
    frappe.require("/assets/ecommerce_business_store_singlevendor/js/ui/jodit.min.js");
    ...
    function crop_default_uploaderimage(field_name){
        ...
        $(imgDialog.$wrapper).find('.img-close').on('click', function () {
            ...
            frappe.confirm(__("Do you want to delete the image?"), () => {
                let url = 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.delete_current_img';
                if (child_docname)
                    url = 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.delete_current_attribute_img';
                frappe.call({
                    method: url,
                    args: {
        ...
        $(imgDialog.$wrapper).find('#saveImages').click(function () {
            ...
            if (length > 0) {
                ...
                if (child_docname) {
                    frappe.call({
                        method: 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api.update_attribute_option_images',
                        args: {
                            dn: child_docname,
