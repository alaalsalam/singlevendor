...
from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.api import getProductCategories,getCustomerList,getAllProductsTitle
...
def get_context(context):
	...
	context.country=frappe.db.sql('''select name from `tabCountry` where enabled=1''',as_dict=1)
	state=frappe.db.sql('''select state from `tabState`''',as_dict=1)
	context.state=state
	general=get_settings_from_domain('Catalog Settings', context.get('domain'))
	if general.default_currency:
		symbol = frappe.get_value('Currency', general.default_currency, 'symbol')
...
@frappe.whitelist(allow_guest=True)
def getAllProducts():
	products = frappe.db.get_all('Product',fields=["*"],filters={"is_active":1},order_by="stock desc")
	if products:
		for i in products:
