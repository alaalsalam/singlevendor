...
from ecommerce_business_store_singlevendor.utils.setup import get_theme_settings
...
def authorize_access(reauthorize=None):
	...
	redirect_uri = get_request_site_address(True) + "?cmd=ecommerce_business_store_singlevendor.utils.google_indexing.google_callback"
...
def get_google_indexing_object():
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.get_google_indexing_object.get_google_indexing_object')
...
def publish_site(url, operation_type="URL_UPDATED"):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.get_google_indexing_object.publish_site')
...
def make_view_log(path, referrer=None, browser=None, version=None, url=None, user_tz=None):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.get_google_indexing_object.make_view_log')
...
def is_tracking_enabled():
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.get_google_indexing_object.is_tracking_enabled')
...
def update_bulk_apiindexing(operation_type='URL_UPDATED'):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.setup.send_indexing_request')
