...
from ecommerce_business_store_singlevendor.ecommerce_business_store_singlevendor.after_install import read_module_path
...
def setup_complete(args):
	...
	if not "SaaS" in args.get('domains'):
		if args.get('sample_data'):
			...
			frappe.enqueue('ecommerce_business_store_singlevendor.utils.setup_wizard.create_sample_data', data_type=data_type)
	frappe.enqueue('ecommerce_business_store_singlevendor.utils.setup_wizard.update_meta_information', args=args)
...
def create_records(data_type='ecommerce', business=None, f_name=None):
	try:
		...
		if file_name:
			path = frappe.get_app_path("ecommerce_business_store_singlevendor")
				...
				for dt in dt_list:
					...
					for i in items_list:
						try:
							...
							frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.utils.setup_wizard.create_records (category creation)')
					opt['created_records'] = success_cnt
					result_json.append(opt)
				frappe.db.commit()
				frappe.enqueue('ecommerce_business_store_singlevendor.utils.setup_wizard.insert_sample_product_1', data_type=data_type, business=business, f_name=f_name, data_result=data_result.get('name'), result_json=result_json)
	except Exception as e:
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.utils.setup_wizard.create_records')
		...
		if os.path.exists(file_path):
			...
			for idx, i in enumerate(products):
				try:
					...
					frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.utils.setup_wizard.insert_sample_product')
