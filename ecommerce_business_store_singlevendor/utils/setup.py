...
from ecommerce_business_store_singlevendor.utils.google_maps import get_geolocation
...
def send_indexing_request(doc, operation_type='URL_UPDATED'):
	try:
		...
		if frappe.db.get_value("Web Theme", settings, "enable_google_indexing"):
			...
			from ecommerce_business_store_singlevendor.utils.google_indexing import publish_site
...
@frappe.whitelist()
def clear_api_log():
	frappe.enqueue('ecommerce_business_store_singlevendor.utils.setup.delete_api_logs')

@frappe.whitelist()
def clear_executed_command_log():
	frappe.enqueue('ecommerce_business_store_singlevendor.utils.setup.delete_executed_command_logs')
...
def delete_api_logs():
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.utils.setup.delete_api_logs')
...
def delete_executed_command_logs():
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'ecommerce_business_store_singlevendor.utils.setup.delete_executed_command_logs')
...
def clear_logs():
	...
	frappe.enqueue('ecommerce_business_store_singlevendor.utils.utils.run_command',
		commands=commands,
		doctype="Website",
		key=key
	)
